...
class EzConfigExportCommands extends DrushCommands {
   ...
   */
  public function export($label = NULL, $options = [
    'add' => FALSE,
    'commit' => FALSE,
    'message' => NULL,
    'destination' => ''
  ...
  public function doExport($options, $destination_dir) {
    ...
    if (count(glob($destination_dir . '/*')) > 0) {
      ...
      if ($config_comparer->createChangelist()->hasChanges()) {
        ...
        if (!$this->io()
          ->confirm(dt('The .yml files in your export directory (!target) will be deleted and replaced with the active config.', ['!target' => $destination_dir]))) {
